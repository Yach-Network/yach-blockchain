...
from yach.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from yach.full_node.full_node_api import FullNodeAPI
from yach.server.start_farmer import service_kwargs_for_farmer
from yach.server.start_full_node import service_kwargs_for_full_node
from yach.server.start_harvester import service_kwargs_for_harvester
from yach.server.start_introducer import service_kwargs_for_introducer
from yach.server.start_service import Service
from yach.server.start_timelord import service_kwargs_for_timelord
from yach.server.start_wallet import service_kwargs_for_wallet
from yach.simulator.start_simulator import service_kwargs_for_full_node_simulator
from yach.timelord.timelord_launcher import kill_processes, spawn_process
from yach.types.peer_info import PeerInfo
from yach.util.bech32m import encode_puzzle_hash
from yach.util.block_tools import BlockTools, test_constants
from yach.util.hash import std_hash
from yach.util.ints import uint16, uint32
...
async def setup_farmer(
    ...
    config["yach_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "yach")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["yach_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "yach")
