...
class TestRpc:
    ...
    async def test1(self, simulation):
        ...
        try:
            ...
            await client.set_reward_targets(encode_puzzle_hash(new_ph, "yach"), encode_puzzle_hash(new_ph_2, "yach"))
            ...
            await client.set_reward_targets(None, encode_puzzle_hash(new_ph_3, "yach"))
            ...
            assert config["farmer"]["yach_target_address"] == encode_puzzle_hash(new_ph, "yach")
            assert config["pool"]["yach_target_address"] == encode_puzzle_hash(new_ph_3, "yach")

            new_ph_3_encoded = encode_puzzle_hash(new_ph_3, "yach")
