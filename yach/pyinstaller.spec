...
ROOT = pathlib.Path(importlib.import_module("yach").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `yach` file name (which is the executable)
    and the `yach` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `yach` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("yach-blockchain"))[0]
...
# TODO: collapse all these entry points into one `yach_exec` entrypoint that accepts the server as a parameter

entry_points = ["yach.cmds.yach"] + [f"yach.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "yach.util.bip39"])

if THIS_IS_WINDOWS:
    yach_mod = importlib.import_module("yach")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/yach/util/english.txt", "yach/util"))
datas.append((f"{ROOT}/yach/util/initial-config.yaml", "yach/util"))
datas.append((f"{ROOT}/yach/wallet/puzzles/*.hex", "yach/wallet/puzzles"))
datas.append((f"{ROOT}/yach/ssl/*", "yach/ssl"))
...
add_binary("yach", f"{ROOT}/yach/cmds/yach.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/yach/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/yach/server/start_{server}.py", COLLECT_ARGS)
