...
from yach.protocols import wallet_protocol
from yach.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from yach.protocols.protocol_message_types import ProtocolMessageTypes
from yach.protocols.wallet_protocol import (
...
from yach.util.byte_types import hexstr_to_bytes
from yach.util.errors import Err, ValidationError
from yach.util.ints import uint32, uint128
from yach.util.keychain import Keychain
from yach.util.lru_cache import LRUCache
from yach.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from yach.util.path import mkdir, path_from_root
from yach.wallet.block_record import HeaderBlockRecord
from yach.wallet.derivation_record import DerivationRecord
from yach.wallet.settings.settings_objects import BackupInitialized
from yach.wallet.transaction_record import TransactionRecord
from yach.wallet.util.backup_utils import open_backup_file
from yach.wallet.util.wallet_types import WalletType
from yach.wallet.wallet_action import WalletAction
from yach.wallet.wallet_blockchain import ReceiveBlockResult
from yach.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]):
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'yach keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSYachConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSYachConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSYachConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSYachConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(self, peer: WSYachConnection, block_i, additions) -> Optional[List[Coin]]:
        if len(additions) > 0:
    ...
    async def get_removals(self, peer: WSYachConnection, block_i, additions, removals) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
